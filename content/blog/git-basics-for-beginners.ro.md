...
Adaugă în „staging" fișierele pe care vrei să le salvezi, apoi fă un commit cu un mesaj scurt care descrie modificarea:
...
`git add .` adaugă tot; poți adăuga și un singur fișier cu `git add cale/catre/fisier`. Un commit este un instantaneu salvat în istoricul tău local. Ca să îl împărtășești, trimite-l pe GitHub:

```bash
git push
```

## Cum aduci modificările altora
