...
```
Each has YAML frontmatter (`title`, `excerpt`, `date` "YYYY-MM-DD", `category`) and a Markdown body. `lib/blog.ts` loads them at build time (`gray-matter` + a `unified` pipeline: `remark-parse`/`remark-gfm`/`remark-rehype` → `lib/highlight.ts` (syntax highlighting) → `lib/rehype/external-links.ts` → `lib/rehype/code-block.ts` → `rehype-stringify`), exposing `getAllPosts(locale)` (sorted newest-first), `getAllSlugs()`, and `getPostBySlug(slug, locale)`. The detail page renders `contentHtml` in a Tailwind `prose` container, wrapped in `components/CodeCopy.tsx`. Each code block gets a header bar (built in `lib/rehype/code-block.ts`) carrying the site's `</>` mark, the language display name, and an empty `<button data-code-copy>` slot; `CodeCopy` portals the lucide icon into that slot on mount and keeps its `aria-label` plus the CSS tooltip's `data-tooltip` in the active locale. Language display names ("Bash", "JavaScript") live in the plugin, not in `messages/*.json` — they are product names, identical in both locales.
